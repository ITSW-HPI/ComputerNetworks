...
\begin{tikzpicture}
  ...
  \draw [->] (0,0) -- (0,5) node [above left, rotate=90] {Attenuation}; 
...
\begin{tikzpicture}[start chain=1, start chain=2, every join/.style={->}]
  ...
  \draw [->, dashed] (tx) to node[above right](signal) {Signal} (rx);

  \node [below=4 of chencode] (bits) {Data bits};
  \node [below=2.5 of tx] (symbols) {Channel symbols};

  \draw [dotted, ->] (bits) edge[out=120,in=270] ($ (sencode)!.5!(chencode) $) edge[out=30,in=270] ($ (chdecode)!.5!(sdecode) $); 
  \draw [dotted, ->] (symbols) edge[out=120,in=270] ($(chencode)!.5!(tx)$) edge[out=30,in=270] ($(rx)!.5!(chdecode)$); 
\end{tikzpicture}
...
\begin{tikzpicture}[start chain=1, start chain=2, every join/.style={->}]
  ...
  \node [below=5.5 of modulate] (bits) {Data bits};
  \node [below=3.5 of tx] (symbols) {Channel symbols};
  \node [below=0.5 of rx.south west] (waves) {Waveforms};

  \draw [dotted, ->] (bits) edge[out=120,in=270] ($ (sencode)!.5!(chencode) $) edge[out=30,in=270] ($ (chdecode)!.5!(sdecode) $); 
  \draw [dotted, ->] (symbols) edge[out=120,in=270] ($(chencode)!.5!(modulate)$) edge[out=30,in=270] ($(demodulate)!.5!(chdecode)$); 
  ...
  \draw [dotted, ->] (waves) edge[out=120,in=270] ($(modulate)!.5!(tx)$) edge[out=30,in=270] ($(rx)!.5!(demodulate)$); 
\end{tikzpicture}
...
\begin{tikzpicture}
  \label{page:fig:channel_freq}
  \node [draw] (sender) {Sender};
  \node [draw, rounded corners, right=of sender] (channel) {Channel};
  \node [draw, right=of channel] (rec) {Receiver};

  \draw [->] (sender) to node[above] {$g(f)$} (channel) to node[above] {$\tilde{g}(f)$} (rec); 
