...
- For a ``1'' bit, apply voltage to the pair of wires, for one second
- For a ``0'' bit, no voltage, for one second 
...
**** Fourier analysis @@latex: \textemdash{} @@ computing coefficients
...
**** Fourier analysis  @@latex: \textemdash{} @@  Compute! 
...
- The transmitted signal of ‘b’ is not a periodic signal  @@latex: \textemdash{} @@   Fourier
...
Characteristic: ``wiggling'' at the points of discontinuity 
...
#+caption: Channel turns transmitted signal $g$ into received signal $\tilde{g}$, represented as frequency response 
#+attr_latex: :width 0.95\textwidth :height 0.6\textheight :options keepaspectratio,page=\getpagerefnumber{page:fig:channel_freq}
#+NAME: fig:fig:channel
[[./standalone/phy.pdf]]

\pause

- Typical channels: modify each harmonic of a signal *separately*
  - By multiplying  and phase-shifting it 
...
- Real channels: different frequencies see different attenuation! 
  - So-called *frequency-selective attenuation* $\alpha(\cdot)$
- Formally: 
...
- (Simplified) *lowpass channel*: Channel has a *cutoff frequency* $f_\mathrm{cutoff}$
  ...
  - All harmonics *above* $f_\mathrm{cutoff}$ experience infinite 
    attenuation  \pause
  - E.g., $\alpha(f') = 2, f'<f_\mathrm{cutoff}$ and $\alpha(f') = \infty$ else 
\pause 
- (Simplified) *highpass channel*: Channel has a cutoff frequency $f_\mathrm{cutoff}$
  ...
  - All harmonics *below* $f_\mathrm{cutoff}$ experience infinite 
    attenuation 
  - E.g., $\alpha(f') =  \infty, f'<f_\mathrm{cutoff}$ and $\alpha(f') = 2$ else 
\pause 
...
**** Channel bandwidth  @@latex: \textemdash{} @@  definition 
...
**** Channels with limited bandwidth  @@latex: \textemdash{} @@  more examples 
...
**** Relationship of signal bandwidth and channel bandwidth  @@latex: \textemdash{} @@  Cases? 
...
#+caption: Signal bandwidth fits into channel bandwidth  @@latex: \textemdash{} @@  desirable situation
...
#+caption: Signal BW larger than channel BW  @@latex: \textemdash{} @@  will lead to quality loss
...
\pause 
- Mostly an issue in optical fibre, indirectly also for wireless signals
...
**** Noise  @@latex: \textemdash{} @@  source? 
- Where does noise come from? 
  - Physics  @@latex: \textemdash{} @@  in particular, random fluctuations of electrons  inside the receiver 
...
**** Noise as addition to received signal 

\begin{tcolorbox}

  \begin{equation}
    \hat{g}(t) = \tilde{g}(t) + \Delta U(t)
    \label{eq:noise_as_addition}
  \end{equation}
\end{tcolorbox}

\noindent with $\tilde{g}$ the channel output, $\hat{g}$ the receiver
output 


**** Power of noise: Instantaneous and average over time 

- Noise  is voltage deviation: $\Delta U(t) = \hat{g}(t) - \tilde{g}(t)$
- *Instantaneous* noise power 
  - Power is proportional to square of voltage ($P = U\cdot I \propto U^2$ )
  - So: Noise  of some  $\Delta U$ has  a power of $(\Delta U)^2$ 
\pause
...
- Challenge: How to get rid of the $T \rightarrow \infty$
  inconvenience? 
...
- $\Delta U$ is random
- Probabilities of any deviation $u$ occurring is described by density
  $f_{\Delta U}(u)$

\pause

***** Rewriting time averages
- Hence,  we can rewrite Eq. \ref{eq:avg_noise_power} as:
...
***** Noise power is variance of random variable 
- But this is just the definition of the variance of the random
  variable ! \pause
- Hence: Noise power is (proportional to) the variance of the noise


**** Detour: From time averages to probability averages 
...
#+caption: Approximating area by looking at each point in time 
#+attr_latex: :width 0.95\textwidth :height 0.4\textheight :options keepaspectratio,page=\getpagerefnumber{page:phy:averages:time}
#+NAME: fig:phy:averages:time
[[./standalone/averages.pdf]]


- Area \approx 2 + 4 + 7+ 6 + 2+ 6 + 4 + 7 + 8 + 6 
- Think: Riemann integral 


***** Averaging across probabilities 
      :PROPERTIES:
      :BEAMER_env: block
      :BEAMER_col: 0.48
      :END:   

#+caption: Approximating area by looking at values and how often they occur  
#+attr_latex: :width 0.95\textwidth :height 0.4\textheight :options keepaspectratio,page=\getpagerefnumber{page:phy:averages:values}
#+NAME: fig:phy:averages:values
[[./standalone/averages.pdf]]


- Area \approx 2\cdot 2 + 2 \cdot 4 + 3 \cdot 6 + 2 \cdot  7 + 1 \cdot  8  
- Think: Lebesgues integral 
...
**** Concrete models for random noise, $\Delta U$  

But *which* density $f_{\Delta U}$ ? 

  - Look at physics, measure effects, count how often how big a deviation occurs 
  - Typical model: $\Delta U$ is  a *Gaussian random variable*
    - Zero mean and some standard deviation \sigma, distribution $N(0, \sigma^2)$, uncorrelated in time
      - Watch out for notation: THIS $N$ is for /normal distribution/,
        *not* /noise/  \pause
    - Variance $\sigma^2$ proportional to temperature (in Kelvin!)

\pause

Hence: *Noise power is proportional to absolute temperature*! 
...
**** Noise  @@latex: \textemdash{} @@  quantitatively?                                      :noexport:
...
- Intuition: 
  ...
  - Receiver takes some time to receive a bit 
    ...
    - For the signal, and the noise  @@latex: \textemdash{} @@  this energy is what REALLY matters 
...
**** Data rate with multi-valued symbols  @@latex: \textemdash{} @@  Nyquist 
...
- So, not unlimited? 
  ...
  - A concrete system can be *bandwidth-limited* or *noise-limited*,
    depending which of the bounds is tight 
...
- The receiver has to be *synchronized* with the sender at the symbol level
  ...
  - There is also ``character'' synchronization  @@latex: \textemdash{} @@  omitted here
...
- Usually, this does not work
  - *Clock drift* is major problem  @@latex: \textemdash{} @@  two  clocks never stay in perfect synchrony 
...
- It should @@latex: \textemdash{} @@ after all, receiver can use 0-1-0 transitions to detect the length of a bit
- But it fails depending on bit sequences: think of long runs of 1s or
  0s  @@latex: \textemdash{} @@ receiver can lose synchronization
- Not nice not to be able to transmit arbitrary data

**** Extract clock information from signal itself @@latex: \textemdash{} @@ Manchester 
...
Recall Figure \ref{fig:phy:channel_bandwidth:outside_signalbw}


#+caption: Signal BW would fit into channel BW, but is at the wrong part of the spectrum; can be cured by proper modulation techniques (Repetition of \ref{fig:phy:channel_bandwidth:outside_signalbw})
#+attr_latex: :width 0.95\textwidth :height 0.6\textheight :options keepaspectratio,page=\getpagerefnumber{page:phy:channel_bandwidth:outside_signalbw}
#+NAME: fig:phy:channel_bandwidth:outside_signalbw_2
[[./standalone/phy.pdf]]
...
- Special cases: 
  - $s(t)$ is an analog signal @@latex: \textemdash{} @@ *amplitude modulation*
  - $s(t)$ is a digital signal @@latex: \textemdash{} @@  *amplitude keying*
  - $s(t)$ only takes 0 and 1 (or 0 and a) as values @@latex: \textemdash{} @@ *on/off keying*

**** Amplitude modulation @@latex: \textemdash{} @@ example 
...
- Watch out for different time scales! 
- Figures above seemed to indicate: period of signal and length of
  symbols is comparable
  - This is but a drawing issue! 
\pause

- Usually, *very* different
  - E.g., WiFi
    - Symbol duration: 4 $\mu$s
    - Signal frequency 5 GHz, hence signal period 0.5 ns 
    - So roughly 1000 signal oscillations per symbol 
...
- The higher the SNR, the better the reception
  ...
  - Actually: *Energy per bit* $E_b$ @@latex: \textemdash{} @@ takes into account data rate, #bits/symbol
...
- A sender has two principal options what types of signals to generate
  - It can choose from a *finite set* of different signals @@latex: \textemdash{} @@ *digital transmission* 
  - There is an *infinite set* of possible signals @@latex: \textemdash{} @@ *analog transmission*
...
- Guided transmission media
  - Copper wire @@latex: \textemdash{} @@ twisted pair
  - Copper wire @@latex: \textemdash{} @@ coaxial cable
  - Fiber optics
...
**** Channel with limited frequency range @@latex: \textemdash{} @@ example 
