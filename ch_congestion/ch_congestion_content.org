...
**** Possible actions  @@latex: \textemdash{} @@ Taxonomy: Timescale
- *Increase capacity*  @@latex: \textemdash{} @@ activate additional links, routers, …
...
- *Reservations* and *admission control*
  ...
  - Feedback about network state only relatively rarely  @@latex: \textemdash{} @@ akin to
    open-loop control
\pause 
...
**** Possible actions  @@latex: \textemdash{} @@ Timescales 
...
- By a *rate*  @@latex: \textemdash{} @@ so and so many bytes per second?
- By a *\gls{CWND}*  @@latex: \textemdash{} @@ as a set of sequence numbers/amount of bytes that may be injected into the network before further permits are received?
...
**** Avoiding full queues  @@latex: \textemdash{} @@ proactive actions?
...
- What will happen?
  ...
  - Packet transmissions driven by ACK arrivals   @@latex: \textemdash{} @@  so-called *ACK
    clock* 
...
- Good news: ACK arrival
  ...
  - Let’s be greedy and try to offer a bit more load  @@latex: \textemdash{} @@ and see if it works!
...
- Overloaded network is bad situation  @@latex: \textemdash{} @@ quick and drastic response necessary
...
**** Additive increase  @@latex: \textemdash{} @@  example 
...
**** Additive increase  @@latex: \textemdash{} @@  Example 2
...
\pause   
  ...
  \node at (cwnd |- aEndReceive_3)  {2.5 + 1/\textcolor{hpired}{2.5} = 2.9 };
  \draw [dotted] (aEndReceive_3) --  (inflight |- aEndReceive_3) node {1};
...
\pause   
  % % ----- nach 3 RTTs
  \node at (cwnd |- aEndReceive_4)  {2.9 + 1/2.9 $\approx$ 3.2};
  \draw [dotted] (aEndReceive_4) --  (inflight |- aEndReceive_4) node {1};
  
  
  \pplusa{3*2 + 3*0.3 + 2*0.05}{hpiblue!30}{0.3}{0}{6}{}

  \node at (cwnd |- aEndReceive_5)  {3.2 + 1/\textcolor{hpired}{3.2} $\approx$ 3.5};
...
**** Additive increase  @@latex: \textemdash{} @@ details 
...
**** AIMD  @@latex: \textemdash{} @@ Sawtooth pattern of TCP’s offered load

Consequences of AIMD
- A TCP connection perpetually probes the network to check for
...
- Example: Transatlantic link 
  - 50 ms delay, 48 Gbit/s bottleneck link rate, TCP packets of 1500
    bytes
    - Delay: about 30 ms propagation, 20 ms processing 
\pause 
- Results
  - Bottleneck rate in packet/s = 48 Gbit/s / 1500 bytes/packet = 48
    Gbit/s / 12000 bits/packet = 4 MPacket / s 
  - CWND = Bottleneck rate \cdot RTT = 4 MPacket/s * 0,1 s = 400.000
    Packets  
...
**** How long to slow start? 

- Slow start's exponential growth of CWND *will* lead to congestion
  and packet loss 
  - Detected by timeouts
- What to conclude? \pause
  - CWND before timeout was too big; must be reduced
- Options:
  - Halve CWND, continue linearly as discussed above is an idea
    - But what to do once the next packet loss is detected? 
  - Slow start gives  an alternative: 
    - Upon *each* packet loss, set CWND to 1
    - Switch to slow start
    - But remember to the CWND when loss was detected
    - This CWND is too big, but half of it *did* work 


**** Slow start threshold 

- Use CWND when loss was detected 
- Remember half of it as *slow-start threshold* (SSTHRESH)
- Use SSTHRESH to terminate exponential CWND growth, switch to
  linear growth
\pause
- Alternate between these two modes
  - *Slow start*: Exponentially grow CWND, start from 1, triggered by
    packet loss 
  - *Congestion avoidance*: Linearly grow CWND, triggered by reaching
    SSTHRESH 
\pause
- Additional advantage: Quick probing of available data rate when
  network conditions adversely changed 
...
- When initializing a connection, no idea about bottleneck rate 
   @@latex: \textemdash{} @@ Have to wait for the first packet loss
  - Or use heuristic assumption
    - Commonly used: Initial CWND=2, 4, \dots, 10; SSTHRESH = 32, 64,
      \dots 64 kBytes (recommended: /arbitrarily high/, RFC 5681)




**** COMMENT Leaving slow start
...
- Suppose 
  ...
  - Packets arrive, but acknowledgements are lost
    - Or: CumAcks arrive, with one packet missing, but are ignored by
      a Tahoe sender 
  - Eventually, timeout occurs, CWND is halved
...
**** Packet bursts  @@latex: \textemdash{} @@  Example 
...
\pause 
  ...
  \pplus{pEndSend-P11}{b}{P12}{P12}{0.05}{0.5}{8}{hpiyellow!60}

\pause 

  % annotate packet burst
  \draw [decorate, decoration={brace, raise=10pt, mirror}] (pStartSend-P8) to node [left=15pt] (burstlabel) {} (pEndSend-P12);
  \node [align=right, anchor=east, left=0 of burstlabel]  (burstdescr) {CWND=5\\no packets in flight\\send 5 packets back to back};
...
**** Packet burst  @@latex: \textemdash{} @@  Example rotated 

#+caption: Packet bursts  @@latex: \textemdash{} @@  rotated version of Figure  \ref{fig:cc:packet_burst_msc:1}
...
- Introduce new mechanism
  - E.g., so-called *pacing*: Explicitly keep track of recent
    inter-ACK arrival times
\pause 
- Improve reaction to CumAcks with gaps 


**** COMMENT Use slow start to avoid packet bursts 

Idea: 
- Upon timeout, *reset CWND to 1 and switch to  slow start again*
- Avoids bursts, yet  quickly ramps up CWND/transmission rate again 
...
- Stay in slow start just *below* that suspected rate (say, half of
  it)
  - Expressed as *slow-start threshold* (SSTHRESH)
...
- Details of previous example varies a bit with implementation; we
  assumed *FastRetransmit* here 
- Plenty of other scenarios with packet bursts
  - Protocols with initial control exchanges, e.g., SCP 
  - Restarting an idle  TCP connection  (i.e., for one timeout duratio)
    - ACK clock no longer available, bottleneck rate might have changed
    - Recommended: restart with initial CWND, slowstart
  - Bursty applications with gaps below restart timer 
  - ACKs queue up in reverse direction and arrive with close to each
    other (*ACK compression*)
  - One ACK for multiple packets
...
\pause 
Actions:  
- Hence: Packet n+1 has to be resent, some overload requires reducing rate, but no severe
...
- Geared towards different environments: large rate & large delay links, wireless
...
- Idea:
  ...
  - More quickly get  close to previous CWND
  - Then, tread carefully around W*
  - If we manage to increase CWND substantially beyond W*, then
    - additional capacity has become available
    - ramp up CWND aggressively 

**** TCP Cubic: CWND as a function of time since packet loss 

- Control CWND as some $W(t)$: 
  - where $t$ is time since last packet loss at $t_0$, with
    $\mathrm{CWND}(t_0) = W^*$ (before reduction)
...
**** TCP throughput in presence of errors  @@latex: \textemdash{} @@ Assumptions 
...
**** TCP throughput in presence of errors  @@latex: \textemdash{} @@ CWND process
...
**** TCP throughput in presence of errors  @@latex: \textemdash{} @@ Throughput
...
- Hence throughput:  
\[  \mathrm{TP}_\mathrm{Reno} = \frac{n}{\text{Period length}}
...
**** Reno throughput  @@latex: \textemdash{} @@  Lessons learned 
