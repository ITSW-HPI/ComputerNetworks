...
#+caption: Example topology for networking (repeated from Chapter 1)
...
*****     Example routing table for node Sw 4                
      :PROPERTIES:
      :BEAMER_env: block
      :BEAMER_col: 0.48
      :END:   
...
\begin{table}[htbp]
\caption{\label{tab:basics:routing_table_sw4}A possible routing table for switch 4}
\centering
\begin{tabular}{llrrll}
Destination & \multicolumn{5}{c}{Neighbor/interface} \\
 & N5  & Sw3 & Sw2  & N4 & N7\\[0pt]
 &  via a &  via b &  via c &  via d &  via e\\[0pt]
\hline
N 1 & - & 4 & 3 & - & -\\[0pt]
N 2 & - & 4 & 3 & - & -\\[0pt]
N 3 & - & 2 & 3 & - & -\\[0pt]
...
| Destination | Interface to neighbor |
|-------------+-----------------------|
| N 1         | c  to Sw2             |
| N 2         | c  to Sw2             |
| N 3         | b  to Sw3             |
...
**** Forwarding table: Orthogonal view 
...
- Alternative, equivalent representation of forwarding table:
  Interface-wise 
  - For each interface, store list of all destinations that are best
    reached via that interface     
\onslide<2->  
- Partially an implementation issue, partially way of thinking 
- Challenge: how to efficiently express such sets of nodes? 
...
#+caption: Forwarding table of Switch 4, represented per-interface 
#+NAME: tab:routing:forwading:orthogonal
| Interfaces to neighbors | Destinations |
|-------------------------+--------------|
| a  to N5                | N 5          |
| b  to Sw3               | N 3          |
| c  to Sw2               | N 1, N2, N4  |
...
**** Timescales: Routing vs. forwarding 
...
- Routing tables (re-)computed upon substantial changes in the network
- Simplest case: Topology changes
  - Devices added, fail
  - Edges added, fail 
- Fancy case: Load changes substantially
- Sometimes, periodically, as fall back 
  - Eg. OSPF: default after 30 minutes 
...
Forwarding table
- *Recomputed* when routing table changes 
- *Used* for *every packet*
  - Might be every few nanoseconds 

*****  Timescales matter!                                      :B_alertblock:
      :PROPERTIES:
...
- Properties/classifications
  ...
  - Stable @@latex: \textemdash{} @@ will converge to an equilibrium over time
  - Optimality (in throughput, delay, … or some other metric)
  - Fairness
  - *Small routing tables* 


**** Properties of forwarding algorithms

- Should use *small, constant* time per packet 
- Assisted by
  - Small forwarding tables
  - Constant packet header size; $O(\log(n))$ bits ok 

**** Storing forwarding tables
- How to store forwarding tables, especially in "orthogonal" view
  (Table \ref{tab:routing:forwading:orthogonal})? 
  - Actually one entry per known destination clearly impossible 
  - Need to compress lists of destinations per interface, group ``similar'' entries 
...
**** Computing routing tables @@latex: \textemdash{} @@ A centralized view
...
- Such that for each pair $(v,u)$ the path $(v, s_1, \dots, s_n, u)$
  with the  smallest (known) cost can be easily derived from the routing tables
...
**** Centralized shortest path @@latex: \textemdash{} @@ Dijkstra 
...
- Once relabeling step is complete, search (in entire graph) tentative node with smallest cost @@latex: \textemdash{} @@ and make it permanent
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example 
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example: Step 0 and 1 
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example: Step 2 and 3
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example: Step 4 and 5 
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example: Step 6 and 7
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example: Step 8 and 9
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example: Step 10 and 11
...
**** Centralized routing @@latex: \textemdash{} @@ Dijkstra @@latex: \textemdash{} @@ Example: Result              :noexport:
...
**** Centralized vs. distributed algorithms @@latex: \textemdash{} @@ Link-state routing
...
\pause 
- Solution: Have each node distribute this information @@latex: \textemdash{} @@ state of all its links @@latex: \textemdash{} @@ in the entire network
...
- Interactive example at [[https://algorithms.discrete.ma.tum.de/graph-algorithms/spp-bellman-ford/index_en.html][Bellman-Ford Algorithm at TUM]]
 

**** Distance-vector routing @@latex: \textemdash{} @@ Formally 
- Suppose that node $X$ 
  - Has distance vector $(x_1, \dots, x_n)$, next-hop vector $(h_1, \dots, h_n)$ for all  $n$ destinations 
    - Unknown distances are assumed to be $\infty$ 
...
- Then: Node $X$ compares, for every destination $i$, whether \[x_i > y_i + c_{X,Y}\]
  - If yes, update: $x_i \leftarrow y_i  + c_{X,Y}$, $h_i \leftarrow Y$
\pause
- Algorithm keeps iterating forever, triggered by updates  

**** Distance-vector routing @@latex: \textemdash{} @@ Example 
...
**** Distance-vector  @@latex: \textemdash{} @@  Sequence of examples 
...
**** Distance vector  @@latex: \textemdash{} @@ Step 1  
...
**** Distance vector  @@latex: \textemdash{} @@ Step 2  
...
**** Distance vector  @@latex: \textemdash{} @@ Step 3
...
**** Distance vector  @@latex: \textemdash{} @@ Step 4
\begin{table}
...
**** Distance vector  @@latex: \textemdash{} @@ Step 5
...
- No changes from step 4 to step 5
  - Done? 
\pause 
- Not done  @@latex: \textemdash{} @@  detecting termination of DV is complex 
...
- But: does a given node really care about details of the route ``far away''?
  - Suppose you send a packet to an Australian university @@latex: \textemdash{} @@ do you care which route it takes from one campus router to the next? 
  - It suffices to get the packet towards Australia
  - For local packets, a router better should care!
- $\leadsto$ *Hierarchical routing*
**** Hierarchical routing @@latex: \textemdash{} @@ Regions 
...
**** Hierarchical routing  @@latex: \textemdash{} @@  Example network
...
**** Hierarchical Routing @@latex: \textemdash{} @@ Example tables 
\vskip-2.5em
...
- *Necessary*: Router must be able to easily infer the target region from packet’s destination!
  ...
  - Treated  Slides \ref{s:network:addr_hierachy}ff
...
- Such conglomerates are fairly independent of each other
  - Different routing protocols can be used @@latex: \textemdash{} @@ an *interior gateway
    protocol* inside each conglomerate 
  - Between gateways, yet another protocol is conceivable @@latex: \textemdash{} @@ an *exterior gateway protocol*
    - operating only on the ``gateway graph''
  - Because of autonomy, the name *autonomous system* is used


**** Addressing @@latex: \textemdash{} @@ Failure of simple addresses
\label{s:network:addr_hierachy}
...
- ``Flat'' addresses @@latex: \textemdash{} @@ addresses that express no structure @@latex: \textemdash{} @@ do not work well together with hierarchical routing
- Necessary: Addresses that express/respect the hierarchical routing structure 
  ...
  - *Hierarchical addresses* @@latex: \textemdash{} @@ addresses are relative to higher groups
...
- *Networks* or *subnets*
  - *Subnets*: easily identifiable subsets of the IP name space 
  - Interfaces know to which subnet they belong
  - Routers usually work on the level of subnets, not individual nodes
    - Only at the very last hop, nodes matter
...
- On previous slide: *interfaces* are  addressed;
  - Or a virtual version thereof; think virtual machines 
  - Still: sometimes, people (incorrectly) just talk about *hosts*
\pause
- Recall router example from early on: Interfaces of the router belong to different networks
 ...
 - \onslide<2-> (Yes, it should be names \dots)
...
**** COMMENT Forwarding using IP addresses/IP routing tables
...
On an example Linux box, from \url{https://ubuntu.com/core/docs/networkmanager/routing-tables} 
...
- Read ~default~ as prefix ~0.0.0.0/0~, the least-specific prefix of
  all 

**** IPv4 prefix example 

- Universität Potsdam: 141.89.0.0/16
- Compare [[https://bgp.tools/prefix/141.89.0.0/16][BGP Tools entry]]
...
**** IPv6 addresses: structure  
  ...
  - Unicast: 64-bit network prefix; 64-bit interface identifier
    - Network prefix subdivided into routing prefix and subnet id;
      similar to IPv4 CIDR ideas
    - Variable prefix length (via slash notation) built in from the start
...
**** IPv6 addresses @@latex: \textemdash{} @@ assignment  
...
- Maybe easier: route @@latex: \textemdash{} @@n
...
**** Relationship of layer 2, 3?  @@latex: \textemdash{} @@  Direct Layer3 neighbors 
...
- Obviously, two packet transmissions necessary (Client  @@latex: \textemdash{} @@  Switch,
  Switch  @@latex: \textemdash{} @@  Server)
  - Which addresses are in these packets?
  - Where do they come from? 
\pause
- Recall: IP Layer 3 sees Client -- Server as *direct neighbors*, is
  unaware of any Layer 2 devices
  - Does not know about Switch A! 
...
**** Sending to next IP hop 

- Assume client already knows the IP destination address 1.2.3.1

***** Next IP hop 
- It needs to find out the next IP hop towards destination 
  - From its routing table
  - Could be another IP node, could already be the destination itself 
  - Then: no more progress *on L3* possible; these are L3 neighbors!  

\pause

***** Sending to next IP hop 

- To send to that neighbor: Pass packet towards L2, ask it to send it
  there 
\pause
- But L2 only understands L2 addresses! 
...
Simple solution: Yell! 
  ...
  - Comprises own MAC address to indicate where to return answer 
...
#+begin_src shell-session 
$ arp -a
? (172.17.12.1) at cc:4e:24:d0:f1:80 on en4 ifscope [ethernet]
smab111ws07.hpi.uni-potsdam.de (172.17.12.19) at f4:8e:38:bb:e7:30 on en4 ifscope [ethernet]
? (172.17.12.149) at 0:d2:b1:d8:23:69 on en4 ifscope [ethernet]
dgipfel3.hpi.uni-potsdam.de (172.17.12.179) at 9c:76:e:53:34:db on en4 ifscope [ethernet]
#+end_src
...
- \gls{DFN}: AS number 680, [[https://bgp.tools/as/680][BGP tools entry for DFN]]
- Deutsche Telekom, Tier 1 carrier: e.g. AS 15996, 2773, [[https://bgpview.io/asn/3320][3320]]
  - 3320: mostly inbound, 8 internet exchanges (at 1  @@latex:
    \textemdash{} @@ 110 GBps), estimated total at 50-100
    TBps, prefixes: a lot 
...
- *Transit AS*: Multiple connections, does carry both local and
  transit traffic @@latex: \textemdash{} @@ typical example of an Internet service provider
  (ISP)
  - Asymmetry between provider and client; money typically flows from
    customer AS to transit AS 
- *Peering points* (or *Internet exchanges*): point of connection
  between many providers
  - Symmetric relationship; usually no money involved 
  - Example IX: [[https://bgpview.io/ix/25][DE-CIX]]
...
#+caption: Active BGP entires, from [[https://www.cidr-report.org/cgi-bin/plota?file=%2fvar%2fdata%2fbgp%2fas2.0%2fbgp%2dactive%2etxt&descr=Active%20BGP%20entries%20%28FIB%29&ylabel=Active%20BGP%20entries%20%28FIB%29&with=step][CIDR-report.org]] 
#+attr_latex: :width 0.95\textwidth :height 0.6\textheight :options keepaspectratio
#+NAME: fig:activebgp
[[./figures/active_bgp_2025.png]]
...
- Main properties
  ...
  - Routing based on traffic type (e.g., real-time traffic uses different paths) @@latex: \textemdash{} @@ different metrics for a link for different traffic 
...
Inter-domain routing is hard
  ...
  - Scale @@latex: \textemdash{} @@ currently, inter-domain routers have to know about 140.000
    networks (rapidly outdated figure\dots)


**** Challenge: Traffic engineering between ASes

- Inbound
  - Control routes via which traffic enters network
  - Might want to steer traffic to other peering points, connections
    to other ASes
- Outbound
  - Control where traffic leaves your AS 
...
- Operation: *Path-based version* of a distance vector protocol
  - Does not only keep track of cost via a given neighbor, but stores *entire paths* to destination ASs
  - *BGP path*: (next hop router, sequence of ASes)
  - More robust, attacks problems like count to infinity
...
- Internally in an AS: *iBGP session*
  - All BGP speakers of an AS participate, all learn all routes 
  - Gateway routers distribute *all known* prefixes to all *all other*
    iBGP routers  (inside their AS)
- Externally, among neighboring AS: *eBGP session*
  ...
  - eBGP peers mutually announce lists of reachable network prefixes 
...
- Advertisements carry *BGP attributes* 
  - In BGP: *prefix + attributes = route*
\pause

- AS-PATH attribute: list of ASs through which the announcement has passed, extended by each gateway router upon receipt
  - Example: 138.16.64/22 announced via AS2 from AS3 to AS1:
    AS-PATH = AS 3,  AS 1


**** BGP routes, NEXT-HOP
- NEXT-HOP attribute:
  - eBGP: interface of router in neighboring AS 
  - iBGP: interface of router in own AS
\pause
- Usage, at border router (assume packet needs to traverse own AS to
  an egress BGP router):
  - Look up NEXT-HOP in BGP table 
  - Look up route towards this NEXT-HOP in the INTRA-AS routing
    protocol; returns on INTRA-AS next-hop IP address
  - Look up outgoing interface / MAC address as usual via ARP 
\pause
- Caution: Plenty of variations here; compare homework assignment 
...
**** Some IP design principles @@latex: \textemdash{} @@ A historical side remark

RFC 1958, based on papers from mid-80s:
- Make sure it works @@latex: \textemdash{} @@ before writing the standard
- Keep it simple
- Make clear choices
- Exploit modularity
...
- Traffic prioritization
  ...
  - Issue: /paid prioritization/ vs. /net neutrality/
    - Neutrality: no blocking, no throttling, no paid prioritization,
