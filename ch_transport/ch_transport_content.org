...
- We will discuss the mechanisms in addition to congestion control that are required to build a reliable, connection-oriented, end-to-end transport protocol 
  ...
  - We will draw heavily on material from the link layer, as many similar problems are solved there in similar fashion  @@latex: \textemdash{} @@ e.g., sliding window for error control
...
**** Transport protocols  @@latex: \textemdash{} @@ Services to upper layer
- Connection-less and connection-oriented transport service
  - Connection management necessary as auxiliary service  @@latex: \textemdash{} @@ setup and teardown
- Reliable or unreliable transport
  - In-order, all packets
- Be a good citizen in the network  @@latex: \textemdash{} @@ perform congestion control
...
- E.g., ``port numbers''
  ...
  - Predefined for ``well-known services''  @@latex: \textemdash{} @@ e.g., port 80 for Web server
...
**** Connection setup  @@latex: \textemdash{} @@ further issues 
...
- Connection setup  @@latex: \textemdash{} @@ SYN (synchronize) packet
- Connection accepted  @@latex: \textemdash{} @@ SYN/ACK packet (because both the previous sequence number is acknowledged and a new sequence number from the receiver is proposed)
- Connection confirmation  @@latex: \textemdash{} @@ ACK packet (combined with DATA, as a rule)
- Problem: SYN attack  @@latex: \textemdash{} @@ flooding with nonsense SYN packets
...
- Coordinated attack
  ...
  - One army is split into two parts that have to attack together  @@latex: \textemdash{} @@ alone they will lose 
...
**** TCP state sequence  @@latex: \textemdash{} @@ unfolding the state machine
...
**** TCP MSC  @@latex: \textemdash{} @@ connection establishment
...
**** TCP MSC  @@latex: \textemdash{} @@ connection release 
...
**** Flow control  @@latex: \textemdash{} @@ buffer allocation
- In order to support outstanding packets, the sender either
  - Has to rely on the receiver to process packets as they come in (packets must not be reordered)  @@latex: \textemdash{} @@ unrealistic, or 
...
**** Flow control  @@latex: \textemdash{} @@ permits and acknowledgements
- Distinguish 
...
- Should be separated in real-world protocols!
  - Unfortunately, often intermixed  @@latex: \textemdash{} @@ TCP, e.g., uses ACKs as Network-Permits; see congestion control chapter
...
- In TCP, receiver can *advertise* size of its receiving buffer
  ...
  - Advertised buffer space *Advertised window* =  MaxRcvdBuffer –
    OccupiedBufferSpace 
    - Buffer space occupied: (NextByteExpected-1)  @@latex: \textemdash{} @@ LastByteRead

\pause 

- Recall: Advertised window limits the amount of data that a sender will inject into the network
  - TCP sender ensures that LastByteSent  @@latex: \textemdash{} @@ LastByteAcked <  AdvertisedWindow 

**** Nagle’s algorithm  @@latex: \textemdash{} @@ self-clocking and windows                 :noexport:
...
- What happens when an ACK for only small amount of data (e.g., 1 byte) arrives?
  - Send immediately? Network will be burdened by small packets (``silly window syndrome'')  @@latex: \textemdash{} @@ inefficient 
...
- Consequence
  - Sender does not have clear picture of which data has already been
    received
    - Compare discussion on FastRecovery in Chapter \ref{ch:congestion}
...
- Example:
  ...
  - With sequence numbers missing: 2781  @@latex: \textemdash{} @@  2900, 2903  @@latex: \textemdash{} @@  2906
- TCP header specifies:
  - CumAck 2780
  - Announce SACK option (takes 2 bytes) 
  - SACK Block 1: 2902  @@latex: \textemdash{} @@  2903
  - SACK Block 2: 2907  @@latex: \textemdash{} @@  2912
...
- Differencen = SampleRTTn  @@latex: \textemdash{} @@ EstimatedRTTn-1 
- EstimatedRTTn = EstimatedRTTn-1 + (\delta Differencen) 
- Deviationn = Deviationn-1 + \delta (|Difference|n  @@latex: \textemdash{} @@ Deviationn-1) 
...
**** Jacobsen/Karels algorithm  @@latex: \textemdash{} @@ Example                           :noexport:
- Same scenario 
...
- Sometimes, timeouts can happen only seconds after a packet has been transmitted  @@latex: \textemdash{} @@ far too sluggish 
...
**** A simple demultiplexer transport protocol  @@latex: \textemdash{} @@ UDP 
...
**** Challenge 2: Head-of-line blocking, example  
...
\pause 
- Bad idea!
  - Incurs connection setup latency for each object
  - These small connections would not even get out of slow start
  - Fairness!
  - OS overhead (separate socket buffers, \dots)
\pause 
- Not used; e.g., Web browsers use at most 6--8 connections (per host) in parallel 
\pause 
...
- Or interfaces like NIC \leftrightarrow  CPU 
...
- Interaction with other protocols
  - Bottleneck bandwidth depends on load of other protocols as well, e.g., UDP  @@latex: \textemdash{} @@ which is NOT congestion-controlled
...
**** TCP  @@latex: \textemdash{} @@ Summary 
- TCP consists of 
  - Reliable byte stream  @@latex: \textemdash{} @@ error control via GoBack-N or Selective Repeat (depending on version)
  - Congestion control  @@latex: \textemdash{} @@ window-based, AIMD, slow start, congestion threshold
  - Flow control  @@latex: \textemdash{} @@ advertised receiver window
  - Connection management  @@latex: \textemdash{} @@ three-way handshake for setup and teardown
...
- They determine to a large degree the dynamics of a network and  @@latex: \textemdash{} @@ in particular  @@latex: \textemdash{} @@ its stability
